...
int main(int argc, char const *argv[])
{
	...
	if(argc < ARG_COUNT){
		printf("Invalid arguments. Correct form: ./<executable name> <no_of_threads>\n");
		exit(0);
	}

	// Read the matrix size from args
	j = 0;
	sscanf(argv[1], "%d", &no_of_threads);
	...
	save_output_to_file("out/sample_sizes_parallel.csv", required_sample_sizes, NO_OF_EXPERIMENTS);
